document.addEventListener("easysearch_rendered", function (e) {
        ...
        easysearch.jq(easysearch.widgetSelectorForCustomSelects)
            ...
            .each(function (e, s) {
                ...
                select.after(
                    ...
                    '<span class="select-styled-prefix">' +
                    index +
                    '</span>' +
                ...
                styled.click(function (e) {
                    console.log('[EasySearch] .select-styled clicked', this);
                    e.stopPropagation();
                    e.stopImmediatePropagation();
                    easysearch
    ...
    function updateOptions(select, styled, optionsList) {
        ...
        var count = select.children("option").length;
        ...
        optionsList.children("li").click(function (c) {
            ...
            var holder = easysearch.jq(this).parent().closest(".easysearch-holder");
            ...
            let styledIndex = styled.closest(".easysearch-select-holder").index();
            setTimeout(function () {
                holder.find(".easysearch-select-holder .select").each(function () {
                    updateOptions(
                        easysearch.jq(this).children("select:first"),
                        easysearch.jq(this).children(".select-styled:first"),
                        easysearch.jq(this).children(".select-options:first")
                    );
                    let nextStyled = holder.find(
                        ".easysearch-select-holder:eq(" + (1 + styledIndex) + ") div.select-styled"
                    );
                    if (nextStyled.length && window.innerWidth > 900) {
                        let nextInput = nextStyled.find(".easyInput");
                        nextStyled.click();
                        nextInput.focus();
                    }
                });
    ...
    if (document.readyState === 'complete' || document.readyState === 'interactive') {
        setTimeout(function () {
            var uninitialized = Array.from(document.querySelectorAll('.easysearch-select-holder > select'))
                .filter(function (sel) {
